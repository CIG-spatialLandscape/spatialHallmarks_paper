...
files <- files[c(1,2,3,5,7,8,9,14,15,16,18,19,20)]
for (file in files) {
...
write.table(df, "Desktop/distance/raw/matrix.txt", sep = "\t")

df <- read.table("Downloads/distances.txt", sep = "\t", header = T)

#fitting
fitting2 <- as.data.frame(matrix(nrow=0, ncol=8))
...
for (tumor in unique(df$sample)) {
  ...
  for (hallmark in unique(df$hallmark)) {
    for (compartment in unique(df$h_type)) {
      ...
      xseq <- seq(from=xrange[1], to=xrange[2], by=4)
      ...
      fitting2 <- rbind(fitting2, loess.DF)
      
    }
  }
}

write.table(fitting2, "Desktop/distance/fitted/matrix2.txt", sep = "\t")
