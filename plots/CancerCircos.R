...
RF_results_Cancer <- sapply(RF_Imp.mt.t, function(x) {
  x = x / sum(x)
})
#set rownames
rownames(RF_results_Cancer) <- rownames(RF_Imp.mt.t)
#transpose again and convert to data frame
RF_results_Cancer <- data.frame(t(RF_results_Cancer))
#set rownames as a new column (ID)
RF_results_Cancer <- tibble::rownames_to_column(RF_results_Cancer, "ID")
#merge importance fraction and metadata and shapley dependencies
RF_results_Cancer <- merge(RF_results_Cancer, RF_Imp.mt[c(1:6, 14:20)], by= "ID")
RF_results_Cancer$type <- factor(RF_results_Cancer$type, levels = c("Breast", "Ovary", "Prostate", "Lung",  "Pancreas", "Bladder", "Colorectal", "Kidney", "Liver", "Glioblastoma"))
RF_results_Cancer <- arrange(RF_results_Cancer, response,type)
#save tumor types names that will split the heatmap
split <- RF_results_Cancer$type
split <- factor(split, levels = unique(RF_results_Cancer$type))

#create a matrix for each information layer of the circos
RF_Imp <- data.matrix(data.frame(RF_results_Cancer[,c(1:8)], row.names = 1))
RF_Cor <- data.matrix(data.frame(RF_results_Cancer[,c(1,14:20)], row.names = 1))
RF_SCD <- data.matrix(data.frame(RF_results_Cancer[,c(1,11)], row.names = 1))
RF_response <- data.frame(RF_results_Cancer[,c(1,13)], row.names = 1)
RF_R <- data.frame(RF_results_Cancer[,c(1,9)], row.names = 1)
RF_sample <- data.frame(RF_results_Cancer[,c(1,10,12,13)], row.names = 1)
